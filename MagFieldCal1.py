...
for zi in range(-50*MMT_dim,-40*MMT_dim,1):
        fi = abs((abs(zi)-500)/10)
        #a[zi] = float("%0.2f" % (0.05 + 0.1*fi))#0.05 + 0.1*fi
        a[zi] = round(0.05 + 0.1*fi,2)
        I[zi] = round(8.e5 -4.e4*fi,2)
        C[zi] = round(mu_0*(I[zi]/pi),2)
        #print(str(zi)+'    '+str(I[zi])+'    '+str(a[zi]))

for zi in range(-40*MMT_dim,40*MMT_dim+1,1):
        a[zi] = 7.50
        I[zi] = 4.e5
        C[zi] = mu_0*(I[zi]/pi)
        #print(str(zi)+'    '+str(I[zi])+'    '+str(a[zi]))

for zi in range(41*MMT_dim,50*MMT_dim+1,1):
        fi = 10-(500-abs(zi))/10
        a[zi] = round(1.05 - 0.1*fi,2)
        I[zi] = round(4.e5 +4.e4*fi,2)
        C[zi] = round(mu_0*(I[zi]/pi),2)
        #print(str(zi)+'    '+str(I[zi])+'    '+str(a[zi]))

XX = range(-50*MMT_dim,50*MMT_dim+1,1)
YY = range(-50*MMT_dim,50*MMT_dim+1,1)
ZZ = range(-50*MMT_dim,50*MMT_dim+1,1)
...
def multicore():
        ...
        with h5py.File('MagFieldData_array_1cm_grid_kaggle.hdf5','w') as hf:
